...
//    { 2650, 2700}  /* V_HIGH       RESET threshold */
    { 3600, 3600}, /* MUTE_HIGH  WARNING threshold */
    { 3600, 3600}  /* V_HIGH       RESET threshold */
};
...
static uint32_t   inter_batt_volt_avg;
static uint16_t   inter_batt_volt_avg_last;
static bool isInterBattIsCharging = false;
/*===========================================================================*
...
{
    return (((uint32_t)(ADC_Read(AD_EXT_BATTERY))) * 100)/(EXT_AD_GAIN()) + DIODE_VOLTAGE_DROP;
}
...
{
    return inter_batt_volt_avg_last;
}
/**********************************************************************
*
...
void Pwr_Fail_Check_Voltage(void)
{
    ...
    inter_batt_volt_avg += int_batt_voltage;
    period_slice ++;
    if(period_slice%4 == 0)//20ms
    {
        batt_volt_avg_last = (batt_volt_avg >> 2);
        inter_batt_volt_avg_last = (inter_batt_volt_avg >> 2);
        batt_volt_avg = 0;
        inter_batt_volt_avg = 0;
    }
...
{
    return isInterBattIsCharging;
}
void Pwr_Fail_Set_Inter_batt_Charge_Sta(bool sta)
{
    isInterBattIsCharging = sta;
}
/**********************************************************************
 *
 * Revision History
