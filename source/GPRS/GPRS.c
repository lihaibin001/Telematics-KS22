...
typedef enum {
	ret_sucess, ret_pass, ret_fail, ret_malloc_err, ret_parm_err, ret_fatal_err,
} FnRet_t;
...
typedef enum {
	StateRecover, StatePowerOff,      //module is power off
...
static FnRet_t moduleFtpDataReceiveHandle(void *pParm);
 ...
 ** Local variable
 **/
static context_t context[CONTEXT_NUM] = { { .contextId = 1, // context id 1 map to TCP context
		.contextState = CONTEXT_STATE_DEACTIVE, // context id state
		.isContexEnable = CONTEXT_ENABLE, .APN = (uint8_t *) "CMIOT" }, };
static netSessionInfo_t netSesionTbl[] = { { .pHost = "101.37.87.65",
		//.pHost = "59.44.43.234",
		.port = 30032, .type = (uint8_t *) "TCP", .state = ipSesionClose,
		.isActive = true, .context = &context[TCP_CMIOT_ID], }, { .pHost =
		"101.37.87.65", .port = 30032, .type = (uint8_t *) "TCP", .state =
		ipSesionClose, .isActive = false, .context = &context[TCP_CMIOT_ID],

}, { .pHost = "101.37.87.65", .port = 22, .type = (uint8_t *) "FTP", .state =
		ipSesionClose, .isActive = false, .context = &context[TCP_CMIOT_ID], } };
static ftpInfo_t ftpInfo = { .pUsr = "derenftp", .pPwr = "deren@123", .pUrc =
		"\\", .pName = "12.txt" };
static uint8_t moduleState;
static moduleInfo_t moduleInfo;
static const uint8_t netSesionNum = sizeof(netSesionTbl)
		/ sizeof(netSessionInfo_t);
...
/* A pointer point to a fucnciont that used to handle the FTP data */
ipRxDataHanle ftpRxDataHandler;
...
static fpEventFnx EventFucntion[EvtCnt] = { modulePowerOnHandle,
		modulePowerOffHandle, moduleSleepHandle, moduleResetHandle,
		moduleTestHandle, moduleCsqCheckHandle, moduleInitializeHandle,
		//	moduleEstbalishWirelessLinkHandle,
		moduleConnectToServerHandle, moduleDeactiveGprsHandle,
		moduleCloseSesionHandle, moduleIpDataReveiveHandle,
		moduleFtpDataReceiveHandle, moduleRecoverSesoinHandle };

//static uint8_t ownIp[4] = "";
static DecodeTbl_t urcTable[UrcNum] = { { "+CREG: ", UrcCreghandler }, {
		"+CGREG: ", UrcCgregHandler }, { "+CMTI: ", UrcCmtiHandler }, {
		"+CMT: ", UrcCmtHandler }, { "+CRING: ", UrcCringHandler }, { "+RDY: ",
		UrcRdyHanler }, { "+CFUN: ", UrcCfunHandler }, { "+CPIN: ",
		UrcCpinHandler }, { "+QIND: ", UrcQindHandler }, { "POWERED DOWN",
		UrcPowerDownHander }, { "+CGEV: ", UrcCgevHandler }, { "+CEREG: ",
		UrcCeregHandler }, { "+QIURC: ", UrcQiurcHandler }, { "+QFTPOPEN: ",
		UrcQftpOpenHandler }, { "+QFTPCLOSE: ", UrcQftpCloseHandler } };
...
static void handlerInit() {
	...
	if (AtCmdQueue == NULL) {
		for (;;) {
			DEBUG(DEBUG_HIGH,
					"[IOT] ERROR: Create AT command queue failed!\r\n");
		}
	}
	...
	csqCheckTimer = xTimerCreate("csq checker", pdMS_TO_TICKS(1000 * 60),
	pdTRUE, (void *) 0, ModuleCsqCheck);
...
static FnRet_t cmdSend(uint8_t *pCmd, uint16_t cmdLen, uint32_t timeOut,
	...
	if (pdPASS != xQueueSend(AtCmdQueue, &cmd, pdMS_TO_TICKS(100))) {
		DEBUG(DEBUG_HIGH,
				"[IOT] ERROR: Push command to command queue error\r\n");
			...
			if (pdPASS == xQueuePeek(TxDataQueue, &data, 0)) {
				if (len != data.dataLen) {
					if (pdPASS == xQueueReceive(TxDataQueue, &data, 0)) {
						...
						DEBUG(DEBUG_HIGH,
								"[IOT] error when delete the at queue item\r\n");
					}
					return;
				}
			}
...
static UrcCdoe_t urcDetecter(uint8_t *pData) {
	...
	for (idx = 0; idx < UrcNum; idx++) {
		if (!memcmp(pData, urcTable[idx].pCode,
				strlen((char *) urcTable[idx].pCode))) {
		...
		for (;;) {
			...
			case 1: //context state
				if (*pResp == '1') {
					...
					context[contextId - 1].contextState =
					CONTEXT_STATE_DEACTIVE;
				}
				break;
			case 2: //context type
				break;
			...
			{
				uint32_t ip[4];
				sscanf((char *) pResp, "%d.%d.%d.%d", &ip[0], &ip[1], &ip[2],
						&ip[3]);
		...
		while (*pData) {
			if (*pData == ',') {
				...
				if (*(pData) == '0') {
					...
					DEBUG(DEBUG_HIGH,
							"[IOT] Open connection error, code:%s\r\n", (pData));
					return ret_fail;
				}
			}
			pData++;
	...
	if (!memcmp(pData, "> ", 2)) {
		...
		if (pdPASS == xQueuePeek(AtCmdQueue, &command, 0)) {
			...
			if (pdPASS == xQueuePeek(TxDataQueue, &Data, 0)) {
				if (Data.pData == NULL) {
					DEBUG(DEBUG_HIGH,
							"[IOT] ERROR: Receive >, but data in queue error!\r\n");
				}
				...
				DEBUG(DEBUG_HIGH,
						"[IOT] ERROR: Receive >, but have no data to sent!\r\n");
			}
		}
...
static FnRet_t fsGetFtpFile(uint8_t *pName) {
...
static FnRet_t ftpDownloadDecoder(uint8_t *pData) {
	...
	if (!memcmp(pData, "+QFTPGET: ", 10)) {
		uint16_t err = atoi((char *) pData + 10);
		uint16_t protocal_err = atoi(strstr((char *) pData + 10, ","));
		if (err != 0) {
			DEBUG(DEBUG_HIGH, "[IOT] FTP get error. err:%d;prot_err:%d", err,
					protocal_err);
			return ret_fail;
		} else {
			//Get file's name in RAM
			atCmd_tbl_t command;
			uint8_t fileName[32] = "";
			uint8_t fileNameLen = 0;
			uint8_t *pCommaPos = NULL;
			if (pdPASS != xQueueReceive(AtCmdQueue, &command, pdMS_TO_TICKS(10))) {
				DEBUG(DEBUG_HIGH,
						"[IOT] ftpDownloadDecoder get at command error!\r\n");
				return ret_fail;
			}
			pCommaPos = strstr((char *) command.pCommand + 12, ",");
			if (pCommaPos == NULL) {
				DEBUG(DEBUG_HIGH,
						"[IOT] ftpDownloadDecoder can not find comma\r\n");
				return ret_fail;
			}
			fileNameLen = pCommaPos - command.pCommand + 11;
			if (fileNameLen > 32) {
				DEBUG(DEBUG_HIGH,
						"[IOT] ftpDownloadDecoder, file name error\r\n");
				return ret_fail;
			}
			memcpy(fileName, command.pCommand + 12, fileNameLen);
			//Get file from module's RAM
			//fsGetFtpFile(fileName, ftpInfo.pUrc);
			return ret_sucess;
		}
	}
	if (!memcmp(pData, "+CME ERROR: ", 12)) {
		DEBUG(DEBUG_HIGH, "[IOT] CME ERROR. err:%d\r\n",
				atoi((char * )pData + 12));
	}
	return ret_fail;
}
...
static FnRet_t querySendState(uint8_t *pResp) {
	...
	if (!memcmp(pResp, "+QISEND:", strlen("+QISEND:"))) {
		sscanf((char *) (pResp + strlen("+QISEND:")), "%d,%d,%d", &len,
				&ackedbytes, &unackedbytes);
		if (unackedbytes == 0) {
...
static void receiveChecker(void) {
	...
	while (getOneByte(&byte)) {
		if (localFlag.isNetReceving) {
			...
			if (ipRxDataIdx == RxTmp.dataLen) {
				if (pdPASS
						!= xQueueSend(RxDataQueue[RxingSesion], &RxTmp,
								pdMS_TO_TICKS(500))) {
			...
			if (findEnd) {
				...
				if (pdPASS == hasCmd && localFlag.cmdState == 1) {
					...
					//if(localFlag.isEcho )
					if (!memcmp(command.pCommand, cmdBuff,
							strlen((char *) cmdBuff))) {
						memset(cmdBuff, 0, MAX_MODLE_CMD_LEN);
						// echo, continue
						continue;
						...
						if (command.decoder) {
							...
							if (ret == ret_sucess) {
								...
								if (pdPASS
										== xQueueReceive(AtCmdQueue, &command,
												pdMS_TO_TICKS(100))) {
									if (command.pCommand) {
										vPortFree(command.pCommand);
										command.pCommand = NULL;
									...
									DEBUG(DEBUG_HIGH,
											"[IOT] Error: when delete the unused command!\r\n");
								}
	...
	if (pdPASS == hasCmd && localFlag.cmdState == 1) {
		...
		if (xTaskGetTickCount() >= cmdExecuteTime) {
			...
			memset(cmdBuff, 0, MAX_MODLE_CMD_LEN);
			cmdExecuteTime = pdMS_TO_TICKS(command.timeOut)
					+ xTaskGetTickCount();
			localFlag.cmdState = 0;
			cmdTryCnt++;
		}
...
static FnRet_t ipOpen(uint8_t channel) {
	...
	uint8_t cmdLen = 0;
	if ((cmdLen = snprintf((char *) ipOpenCmd, 128,
			"AT+QIOPEN=%d,%d,\"%s\",\"%s\",%d,0,1\r\n",
			netSesionTbl[channel].context->contextId, channel,
			(char *) netSesionTbl[channel].type,
			(char *) netSesionTbl[channel].pHost, netSesionTbl[channel].port))
			>= 127) {
		DEBUG(DEBUG_HIGH, "[IOT] Error: When encode IP open command");
		return ret_fail;
	}
	if (cmdSend(ipOpenCmd, cmdLen, 7500, MAX_RECONNECT_CNT, ipOpenDecoder)
			!= ret_sucess) {
		return ret_fail;
	}
	return ret_sucess;
...
#if 0
static FnRet_t fsGetFile(uint8_t ){
	return ret_fail;
}
#endif
static FnRet_t netInit(void) {
	...
	cmdSend("AT+CPIN?\r\n", strlen("AT+CPIN?\r\n"), CMD_COMMON_INTERVAL,
	CMD_QUERY_SIM_TIMEOUT / CMD_COMMON_INTERVAL, isSimCardReady);
	/* query CS service */
	cmdSend("AT+CREG?\r\n", strlen("AT+CREG?\r\n"), CMD_COMMON_INTERVAL,
	CMD_QUERY_CS_TIMEOUT / CMD_COMMON_INTERVAL, checkCreg);
	/* query PS service */
	cmdSend("AT+CGREG?\r\n", strlen("AT+CGREG?\r\n"), CMD_COMMON_INTERVAL,
	CMD_QUERY_PS_TIMEOUT / CMD_COMMON_INTERVAL, checkPSRegest);
	/* get IMEI */
	cmdSend("AT+GSN\r\n", strlen("AT+GSN\r\n"), CMD_COMMON_INTERVAL, 1,
			imeiDeocder);
	/* get IMSI */
	cmdSend("AT+CIMI\r\n", strlen("AT+CIMI\r\n"), CMD_COMMON_INTERVAL, 1,
			imsiDecoder);
...
//	cmdSend("AT+CGREG?\r\n", strlen("AT+CGREG?\r\n"),300, 60000/300);
	...
	cmdSend(cmd, strlen((char *) cmd), CMD_COMMON_INTERVAL, 1,
			commonRespDecoder);
	/* active PDP context */
	memset(cmd, 0, MAX_CMD_LEN);
...
static FnRet_t ftpInit(void) {
	...
	if (netSesionTbl[FTP_SESION_NUM].context->contextState
			== CONTEXT_STATE_DEACTIVE) {
		memset(cmd, 0, MAX_CMD_LEN);
		snprintf((char *) cmd, MAX_CMD_LEN,
				"AT+QICSGP=%d,1,\"%s\",\"\",\"\",1\r\n",
				netSesionTbl[FTP_SESION_NUM].context->contextId,
				netSesionTbl[FTP_SESION_NUM].context->APN);
		cmdSend(cmd, strlen((char *) cmd), CMD_COMMON_INTERVAL, 1,
				commonRespDecoder);
		/* active PDP context */
		memset(cmd, 0, MAX_CMD_LEN);
	...
	snprintf((char *) cmd, MAX_CMD_LEN, "AT+QFTPOPEN=\"%s\",%d\r\n",
			netSesionTbl[FTP_SESION_NUM].pHost,
			netSesionTbl[FTP_SESION_NUM].port);
...
static FnRet_t ftpDownLoad(uint32_t offset, uint16_t size) {
	uint8_t cmd[MAX_CMD_LEN] = "";
	snprintf((char *) cmd, MAX_CMD_LEN, "AT+QFTPGET=\"%s\",\"RAM:%s\"%d,%d\r\n",
			ftpInfo.pName, ftpInfo.pName, offset, size);
	cmdSend(cmd, strlen((char *) cmd), CMD_COMMON_INTERVAL, 1,
			ftpDownloadDecoder);
}
...
static void ipTransmitter(void) {
	...
	if (netSesionTbl[data.channel].state == ipSesionOpened) {
		if (cmdSend(cmd, strlen((char *) cmd), IP_SEND_TIMEOUT, 1,
				ipSendRespDecoder) != ret_sucess) {
...
static FnRet_t moduleFtpDataReceiveHandle(void *pParm) {
	...
	if (xQueueReceive(RxDataQueue[sesionNum], &data, 0) == pdPASS) {
		if (data.pData != NULL) {
			if (ftpRxDataHandler) {
				ftpRxDataHandler(data.pData, data.dataLen, sesionNum);
			}
			vPortFree(data.pData);
			data.pData = NULL;
			return ret_sucess;
...
static FnRet_t moduleRecoverSesoinHandle(void *pParm) {
	...
	for (sesionIdx = 0; sesionIdx < netSesionNum; sesionIdx++) {
		if (netSesionTbl[sesionIdx].isActive) {
			if (netSesionTbl[sesionIdx].context->contextState
					== CONTEXT_STATE_DEACTIVE) {
...
void IOT_Task(void *pParm) {
	...
	for (;;) {
		Event_t event;
		if (xQueueReceive(EventQueue, &event,
				pdMS_TO_TICKS(TASK_WAIT_MAX_TIME)) == pdPASS) {
...
bool IOT_IpNetSend(uint8_t channel, uint8_t *pData, uint16_t len, ipSendCb cb) {
	...
	if (IOT_GetSessionState(channel) == ipSesionOpened
			&& !localFlag.powerDownReq) {
		TxData_t data;
		...
		if (pdPASS == xQueueSend(TxDataQueue, &data, pdMS_TO_TICKS(500))) {
			...
			DEBUG(DEBUG_HIGH,
					"[IOT] ERROR: When Add Data to tx data queue!\r\n");
			vPortFree(data.pData);
		}
	}
	return false;
