...
typedef struct{
	uint8_t rxDisable : 1;
	uint8_t txDisable : 1;
	uint8_t sesionTypeLock : 1; //sesion type buffer lock
}localFlag_t;
...
#define UDS_FC_BS					(uint8_t)8		//block size
#define UDS_FC_ST					(uint8_t)20		//separation time min
#define UDS_FC_WFT					(uint8_t)0		//Maximum number of flow control Wait frame transmissions
#define UDS_N_AS					(uint8_t)70		//
#define UDS_N_AR					(uint8_t)70
#define UDS_N_BS					(uint8_t)150
#define UDS_N_BR					(uint8_t)70
#define UDS_N_CS					(uint8_t)70
#define UDS_N_CR					(uint8_t)150
#define UDS_P2C						(uint8_t)50
#define UDS_P2EC					(uint16_t)5000
#define UDS_P2S						(uint8_t)150
#define UDS_P2ES					(uint16_t)5100
#define UDS_S3S						(uint16_t)5000
#define UDS_S3C						(uint16_t)4000
/*USER DEFINITION*/
...
#define UDS_SRV_TYPE_DDDI			(uint8_t)0x2C	//Dynamically Define Data Identifier
#define UDS_SRV_TYPE_CDI			(uint8_t)0x14	//Clear diagnostic information
#define UDS_SRV_TYPE_RDTCI			(uint8_t)0x19	//Read DTC information
#define UDS_SRV_TYPE_IOCID			(uint8_t)0x2F	//Input output control by dientifier
#define UDS_SRV_TYPE_RC				(uint8_t)0x31 	//rountine control
#define UDS_SRV_TYPE_RD				(uint8_t)0x34	//Request download
#define UDS_SRV_TYPE_TD				(uint8_t)0x36	//Transfer data
#define UDS_SRV_TPPE_RTE			(uint8_t)0x37	//Request transfer exit
//dtc related
#define UDS_SRV_TYPE_DTC_PT			(uint8_t)0x01	//Request current powertrain diagnostic data
#define UDS_SRV_TYPE_DTC_DFD		(uint8_t)0x02	//Request powertrain freezen frame data
#define UDS_SRV_TYPE_DTC_ERC		(uint8_t)0x03	//Request emission-related diagnostic trouble codes
#define UDS_SRV_TYPE_DTC_ERI		(uint8_t)0x04	//Clear/Reset emission-related diagnostic information
#define UDS_SRV_TYPE_DTC_OSM		(uint8_t)0x05	//Request oxygen sensor monitoring test result
#define UDS_SRV_TYPE_DTC_OBM		(uint8_t)0x06	//Request on-board monitoring test result for specific monitored systems
#define UDS_SRV_TYPE_DTC_ERCL		(uint8_t)0x07	//Request emission-related diagnostic trouble codes detect during current or last competed drving cycle
#define UDS_SRV_TYPE_DTC_COBS		(uint8_t)0x08	//Request control of on-board system, test or component
#define UDS_SRV_TYPE_DTC_VI			(uint8_t)0x09	//Request vehicle information
//#define UDS_SRV_TYPE_DTC_ERPD		(uint9_t)0x0A	//Request emission-related "permanent" diagnostic trouble codes stored.
...
static localFlag_t localFlag;		//store some flag
/*********************************************************************/
...
/**********************************************************************
 *    Function: diag_uds_ecuReset
 *  Parameters: 'pFrame' point to the frame'd received
 *     Returns: None
 * Description: session control service
 *********************************************************************/
static bool diag_uds_ecuReset() {
	if (uds_rx_buffer.length != 2) {
		diag_uds_errorHandle(uds_rx_buffer.payload[0], 0x13);
		return false;
	}
	...
	case 0x04:	//enableRapidPowerShutDown
		break;
	case 0x05:	//disableRapidPowerShutDown
		break;
	default:
		//sub-function unsupported
		diag_uds_errorHandle(uds_rx_buffer.payload[0], 0x12);
...
typedef struct{
	uint8_t comType : 2;
	uint8_t ISOSAEReserved : 2;
	uint8_t disEn : 4;
}comCtrl;
/**********************************************************************
 * Function: diag_uds_communicationControl
 * Returns: None
 * Description: secrity access service
 *********************************************************************/
static bool diag_uds_communicationControl(void) {
	if(uds_rx_buffer.length != 7){
		diag_uds_errorHandle(uds_rx_buffer.payload[0], 0x13);
	}
	comCtrl *pTmp = (comCtrl *)&uds_rx_buffer.payload[3];
	switch(uds_rx_buffer.payload[2]){
	case 0://enableRxAndTx
		...
		case 2://networkManagementCommunicationMessages
			break;
		case 3://networkManagementCommunicationMessages and normalCommunicationMessages
			break;
		default:
			break;
		}
		break;
	case 1://enableRxAndDisableTx
		switch(pTmp->comType){
		case 1://normalCommunicationMessages
			break;
		case 2://networkManagementCommunicationMessages
	...
	case 3://disableRxAndTx
		...
		break;
	case 4://enableRxAndDisableTxWithEnhancedAddressInformation
		switch(pTmp->comType){
		case 1://normalCommunicationMessages
			break;
		case 2://networkManagementCommunicationMessages
		...
		break;
	case 5://enableRxAndTxWithEnhancedAddressInformation
		switch(pTmp->comType){
		case 1://normalCommunicationMessages
			break;
		case 2://networkManagementCommunicationMessages
		...
		break;
#if 0
	case 6://ISOSAEReserved
		break;
	case 0x40://vehicleManufacturerSpecific
		break;
#endif
	default:
		diag_uds_errorHandle(uds_rx_buffer.payload[0], 0x12);
...
static bool diag_uds_readDataById(void) {
	if (uds_rx_buffer.length != 3) {
		...
		uint16_t id = (uds_rx_buffer.payload[1] << 8)
				| uds_rx_buffer.payload[2];
		switch (id) {
...
/**********************************************************************
 * Function: diag_uds_serviceHanndle
 * Returns: None
...
static bool diag_uds_serviceHanndle(void) {
	...
	case UDS_SRV_TYPE_ECURESET :
		return diag_uds_ecuReset();
	case UDS_SRV_TYPE_SECURITY_ACS : //security access
		return diag_uds_securityAccess();
	case UDS_SRV_TYPE_COM_CTRL :
		return diag_uds_communicationControl();
	case UDS_SRV_TYPE_TESTER_PRESENT :
		break;
	case UDS_SRV_TYPE_CTRL_DCT_SET :
		break;
	case UDS_SRV_TYPE_RD_BY_ID : //read data by id
		return diag_uds_readDataById();
	case UDS_SRV_TYPE_RD_MEM_BY_AD :
		break;
	case UDS_SRV_TYPE_WR_BY_ID : //wirte data by id
		return diag_uds_writeDataById();
	case UDS_SRV_TYPE_WR_MEM_BY_AD :
		break;
	case UDS_SRV_TYPE_RD_BY_PDID :
		break;
	case UDS_SRV_TYPE_DDDI :
		break;
	case UDS_SRV_TYPE_CDI :
		break;
	case UDS_SRV_TYPE_RDTCI :
		break;
	case UDS_SRV_TYPE_IOCID :
		break;
	case UDS_SRV_TYPE_RC :
		break;
	case UDS_SRV_TYPE_RD :
		break;
	case UDS_SRV_TYPE_TD :
		break;
	case UDS_SRV_TPPE_RTE :
		break;
	default:
...
static bool diag_uds_parseData(uint8_t *pFrame) {
	...
	case UDS_DATA_TYPE_CONSECUTIVE :
		if (sn == 0) {
			//If received conscutvie frame but the serail number is zero, response conditons not correct
			diag_uds_errorHandle(pFrame[2], 0x22);
			break;
		}
		if (uds_rx_buffer.length > UDS_FIRSR_FRAME_MAX_LEN) {
			...
			sn++;
			memcpy(&uds_rx_buffer.payload[uds_rx_buffer.receivedLength],
					&pFrame[1], 7);
...
#endif
	TMR_Start_Timer(DIAG_BASE_TIMER, PS_STDBY_TIME3, null_action);
	UDS_GetData(&frame);
